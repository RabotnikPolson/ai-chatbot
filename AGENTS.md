...
- Before first start, create local env from template: `Copy-Item .env.example .env` (PowerShell) and set `SECRET_KEY` (`README.md`, `.env.example`).
...
- Dependency health check: `curl http://localhost:8000/health` (expects `200` + `{"status":"all systems go", ...}`).
...
- Worker tests cover idempotency when message is already `done` and successful persistence of `provider`/`latency_ms` on completion (`backend/tests/test_worker.py`).
...
- Inside Docker network, Postgres is reachable by both `db` and alias `postgres` (`docker-compose.yml`, `.env.example`).
