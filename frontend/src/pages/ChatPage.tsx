    ...
    const streamMessage = async (messageId: string, chatId: number, signal?: AbortSignal) => {
        ...
        try {
            ...
            if (error instanceof Error && error.name === 'AbortError') {
                console.log('Stream stopped on chat switch');
            } else {
    ...
    const handleSendMessage = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (targetChatId === null) {
                const title = text.split(' ').slice(0, 4).join(' ') || 'New chat';
            ...
            <div className="p-4 flex items-center justify-between shadow-sm z-10">
                <div className="flex items-center gap-3">
                    <h2 className="text-lg font-medium text-gray-200">
                        {activeConversationId === null ? 'VibeChat' : (currentChat?.title || 'Untitled chat')}
                    </h2>
                </div>
                <div className="flex items-center gap-3">
            ...
            <div className="flex-1 overflow-y-auto px-4 lg:px-48 xl:px-64 pt-6 space-y-8 flex flex-col pb-4 custom-scrollbar">
                {activeConversationId === null && messages.length === 0 ? (
                    <div className="flex-1 flex flex-col items-center justify-center text-center px-4 mt-20">
                        ...
                        <h1 className="text-3xl font-medium text-gray-200 mb-2">How can I help?</h1>
                        <p className="text-[#a8a8a8] max-w-md">I am your local AI chatbot. Type your question below to start.</p>
                    </div>
                ) : (
                    messages.map((msg) => (
                        msg.role === 'user' ? (
                            ...
                            <div key={msg.id} className="flex justify-start w-full gap-4">
                                ...
                                <div className="text-[#e3e3e3] text-[15px] leading-relaxed max-w-[85%] mt-1 overflow-hidden flex flex-col gap-2">
                                    <div className="prose prose-invert max-w-none break-words overflow-x-auto">
                                        ...
                                        >
                                            {msg.content || (msg.status === 'failed' ? 'Response generation failed.' : 'Waiting for response...')}
                                        </ReactMarkdown>
                                    </div>
                ...
                <form onSubmit={handleSendMessage} className="relative">
                    ...
                    <div className="text-center mt-3">
                        <span className="text-[11px] text-[#A8A8A8]">
                            VibeChat is AI. It can make mistakes. Verify important info.
                        </span>
                    </div>
                </form>
            </div>
