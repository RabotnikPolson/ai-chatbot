...
function WsStatusDot() {
    ...
    if (status === 'connecting') return <Loader2 size={13} className="text-yellow-500 animate-spin" />;
...
function Logo() {
    return (
        <div className="flex items-center gap-2.5 px-1">
            <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-[#4f8ef7] to-[#7c3aed] flex items-center justify-center shadow-lg shadow-blue-900/30 shrink-0">
            ...
            <div>
                <p className="text-sm font-semibold text-[#e8e8e8] tracking-tight leading-none">VibeChat</p>
...
export default function Sidebar({ isOpen }: SidebarProps) {
    ...
    // ── Prevent duplicate new chats ───────────────────────────────────────────
    const handleNewChat = () => {
        const existingEmpty = chats.find(
            (c) => c.title === 'Новый чат' && messages.filter((m) => m.chatId === c.id).length === 0
        );
        if (existingEmpty) {
    ...
    if (!isOpen) {
        return (
            <aside className="flex flex-col items-center w-14 shrink-0 h-full bg-[#111111] border-r border-[#1e1e1e] py-4 gap-3">
                <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-[#4f8ef7] to-[#7c3aed] flex items-center justify-center shadow-lg shadow-blue-900/30 shrink-0">
                    <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                        <path
                            d="M8 2C4.68 2 2 4.68 2 8s2.68 6 6 6 6-2.68 6-6-2.68-6-6-6zm0 2.5c.83 0 1.5.67 1.5 1.5S8.83 7.5 8 7.5 6.5 6.83 6.5 6 7.17 4.5 8 4.5zm0 7c-1.25 0-2.36-.63-3.03-1.59.86-.47 1.91-.73 3.03-.73s2.17.26 3.03.73C10.36 10.87 9.25 11.5 8 11.5z"
                ...
                <button
                    ...
                    title="Новый чат"
                    className="w-8 h-8 flex items-center justify-center rounded-xl border border-[#2a4a70] text-[#7bb3ff] hover:text-white hover:bg-[#1e3a5f] transition-all duration-200"
                >
