...
def test_send_message(client):
    ...
    conv_resp = client.post("/conversations/", json={"title": "Messages test chat"}, headers=headers)
...
def test_send_message_with_temperature(client):
    ...
    conv_resp = client.post("/conversations/", json={"title": "Temperature test chat"}, headers=headers)
    conv_id = conv_resp.json()["id"]
    ...
    with patch("services.chat_service.generate_reply.delay") as mock_task, \
        ...
        response = client.post(
            f"/conversations/{conv_id}/messages",
            json={"text": "Generate a reply", "temperature": 0.2},
            headers=headers
        )
...
def test_get_message_status(client):
    ...
    conv_resp = client.post("/conversations/", json={"title": "Chat 2"}, headers=headers)
    conv_id = conv_resp.json()["id"]
...
def test_retry_creates_new_assistant_message(client, db_session):
    ...
    conv_resp = client.post("/conversations/", json={"title": "Retry test chat"}, headers=headers)
    conv_id = conv_resp.json()["id"]
...
def test_retry_rejects_non_failed_assistant_message(client):
    ...
    conv_resp = client.post("/conversations/", json={"title": "Retry guard chat"}, headers=headers)
    conv_id = conv_resp.json()["id"]
