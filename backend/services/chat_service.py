...
class ChatService:
    ...
    def send_message(
        ...
        if cached_history_raw:
            try:
                ...
                redis_client.setex(cache_key, 180, json.dumps(cached_history))
            except json.JSONDecodeError:
                redis_client.delete(cache_key)

        req_id = str(uuid.uuid4())
        json_logger.info("Message queued", extra={
            "custom_fields": {
                "request_id": req_id,
                "user_id": user_id,
                "conversation_id": conversation_id,
    ...
    def retry_message(db: Session, message_id: UUID, user_id: int) -> Message:
        ...
        if message.role != MessageRoleEnum.assistant:
            raise HTTPException(status_code=400, detail="Only assistant replies can be retried")
        if message.status != MessageStatusEnum.failed:
            raise HTTPException(status_code=400, detail="Retry is allowed only for failed assistant messages")
