...
def generate_reply(self, message_id: int, temperature: float = 0.7):
    with SessionLocal() as db:
        try:
            ...
            if cached_history:
                logger.info(f"CACHE HIT: Loaded history from Redis for chat {msg.conversation_id}")
                messages_for_ollama = json.loads(cached_history)
            else:
                logger.info(f"CACHE MISS: Loading history from DB for chat {msg.conversation_id}")
                ...
                redis_client.setex(cache_key, 180, json.dumps(messages_for_ollama))
            ...
            if cached_history:
                try:
                    ...
                    redis_client.setex(cache_key, 180, json.dumps(current_cache))
            ...
            json_logger.info("Reply generation completed", extra={
                "custom_fields": {
                    "request_id": "worker",
            ...
            if is_network_error and self.request.retries < self.max_retries:
                ...
                json_logger.warning(
                    f"Network error during generation, retry attempt ({self.request.retries + 1}/{self.max_retries}), "
            ...
            json_logger.error(f"Reply generation failed: {str(e)}", extra={
                "custom_fields": {
                    "request_id": "worker",
